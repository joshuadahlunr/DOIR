...
namespace doir::Calculator {
	...
	void print(TrivialModule& module, ecs::entity_t root, size_t depth /* = 0 */) {
		...
		if(module.has_component<comp::constant>(root))
			...
			auto name = var.lexeme.view(module.buffer);
			nowide::cout << "var:" << std::string_view{fp_view_data(char, name), fp_view_size(name)};
			...
			auto name = var.name.view(module.buffer);
			nowide::cout << op.left << " (" << std::string_view{fp_view_data(char, name), fp_view_size(name)}
				<< ")" << " assign" << PRINT_ENDER;
			print(module, op.right, depth + 1);
			...
			auto name = var.name.view(module.buffer);
			nowide::cout << "define:" << std::string_view{fp_view_data(char, name), fp_view_size(name)} << PRINT_ENDER;
	...
	void lookup_variables(TrivialModule& module, ecs::entity_t root) {
		...
		for(size_t size = module.entity_count(); e < size; ++e) {
			...
			if(!res) {
				nowide::cerr << "Failed to lookup variable: " << fp_string_view_make_dynamic(name.view(module.buffer)) << std::endl;
				exit(1);
			}

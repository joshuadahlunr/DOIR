	...
	*/
	...
	namespace detail {
		...
		inline entity get_entity(scene::component_storage& storage, scene& scene, size_t index, std::optional<size_t> component_id = {}) {
			...
			else return get_entity<Tcomponent>(scene, index);
		}
	}
...
#define ECS_SWAP_BODY(_swap, _component_id, _Tcomponent)\
	...
	entity eA = detail::get_entity<_Tcomponent>(*this, scene, a, component_id);\
	entity eB = detail::get_entity<_Tcomponent>(*this, scene, b, component_id);\
...
\
	...
	inline bool scene::component_storage::swap(struct scene& scene, size_t _component_id, size_t a, std::optional<size_t> _b /*= {}*/, std::vector<std::byte>& buffer /*= {}*/, bool swap_if_one_elementless /*= false*/) {
		ECS_SWAP_BODY(swap, _component_id, detail::void_like);
		return true;
	}
#undef ECS_SWAP_BODY
	...
	*/
	...
	inline void scene::component_storage::sort(struct scene& scene, size_t component_id, const F& comparator) {
		sort_impl<detail::void_like, F, with_entities>(this, scene, comparator, component_id);
	}
	template<typename Tcomponent, typename F, bool with_entities /*= false*/>
	inline void scene::component_storage::sort(struct scene& scene, const F& _comparator) {
		if constexpr(with_entities) {
			...
			sort_impl<Tcomponent, decltype(comparator), with_entities>(this, scene, comparator);
		} else {

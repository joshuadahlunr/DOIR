...
TEST_SUITE("JSON") {
	...
	TEST_CASE("doir::JSON::Array") {
		...
		ankerl::nanobench::Bench().run("doir::JSON::Array", []{
			...
			auto dump = doir::JSON::dump(module, root);
			CHECK(fp_string_equal(dump, "[true,false,null,5.500000,\"Hello\"]"));
			fp_string_free(dump);
	...
	TEST_CASE("doir::JSON::Object") {
		...
		ankerl::nanobench::Bench().run("doir::JSON::Object", []{
			...
			auto dump = doir::JSON::dump(module, root);
			CHECK(fp_string_equal(dump, "{\"bool\":true,\"!bool\":false,\"null\":null,\"num\":5.500000,\"str\":\"Hello\"}"));
			fp_string_free(dump);
